...
public class EvolutionEngine {
     ...
     * @param timePools  A categorized map of available time slots.
     * @param roomPools  A categorized map of available rooms.
     * @param cancelCheck [NEW]: A hook to safely abort the evolution loop if triggered by the user.
     * @param progressCallback [NEW]: A callback to stream generation metrics back to the UI in real-time.
    ...
    public ArrayList<TimeTable> evolveGenerations(ArrayList<TimeTable> population,
        ...
        for (int gen = 1; gen <= config.maxGenerations; gen++) {
            ...
            System.out.println("Generation " + gen + " | Best Fitness: " + currentBestPenalty + " | MutationRate: " + currentMutationRate);
            ...
            if (progressCallback != null) {
                ...
                progressCallback.accept(new ProgressSnapshot(
                        ...
                        (int) currentBestPenalty, // Casted to int to match the Snapshot DTO
            ...
            long currentBestFitness = currentBestPenalty;
            ...
            if (unevolvedGenerations > 0 && (unevolvedGenerations % stagnationThreshold) == 0 && currentMutationRate < 0.5) {
                ...
                System.out.println("  [Adaptive Engine] Mutation Boost Triggered! New Rate: " + currentMutationRate); // For debugging purposes
            }
