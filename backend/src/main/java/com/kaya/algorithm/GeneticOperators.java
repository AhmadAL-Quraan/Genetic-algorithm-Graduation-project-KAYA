    ...
    public static TimeTable crossover(TimeTable p1, TimeTable p2) {
        ...
        */
        ...
        for (int i = 0; i < p1.getLectures().size(); i++) {
            ...
            childLectures.add(new Lecture(source.getId(), source.getCourse(), source.getRoom(), source.getTimeSlot(), source.getTeacher(), source.getSectionNumber(), source.getInstructor()));
        }
        return new TimeTable(childLectures);
    }
