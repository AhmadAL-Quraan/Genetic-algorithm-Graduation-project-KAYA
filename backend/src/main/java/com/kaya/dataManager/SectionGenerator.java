...
public class SectionGenerator {
    ...
    public static void generate(TimeTable table) {
        ...
        for (int i = 1; i < lectures.size(); i++) {
            if (Objects.equals(lectures.get(i).getCourse().getCourseSymbol(), lectures.get(i-1).getCourse().getCourseSymbol())
                    && Objects.equals(lectures.get(i).getCourse().getCourseNumber(), lectures.get(i-1).getCourse().getCourseNumber())) {
                number++;
            }
            else {
                number = 1;
