    ...
    public ResponseEntity<byte[]> buildTimetableExport(TimeTable tt) throws Exception {
        ...
        List<Lecture> scheduled = tt.getLectures().stream()
                ...
                .sorted(Comparator
                        .comparingInt((Lecture l) ->
                                dayPatternHelper.groupOf(l.getTimeSlot().getDays()))
                        .thenComparing(l -> courseCode(l.getCourse()))
                        .thenComparing(l -> l.getTimeSlot().getStartTime())).toList();
        ...
        try (Workbook wb = new Workbook(out, "KAYA Scheduler", "1.0")) {
            ...
            for (Lecture lec : scheduled) {
                int group = dayPatternHelper.groupOf(lec.getTimeSlot().getDays());
