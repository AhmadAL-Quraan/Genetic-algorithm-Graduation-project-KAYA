...
export function CalendarInner({
  ...
  const [activeDayDate, setActiveDayDate] = useState<Date>(
      dayDateFor("MONDAY")
  );
  ...
  async function applyMove(arg: EventDropArg | EventResizeDoneArg) {
    ...
    const dayDelta =
        ((newDayIdx - oldDayIdx) % 7 + 7) % 7;

    const oldDays = lecture.timeSlot.days;

    const newDays: DayOfWeek[] = oldDays.map(
        d => INDEX_TO_DAY[
        (DAY_TO_INDEX[d] + dayDelta) % 7
            ]
    );
    ...
    try {
      ...
      await updateLectureAssignment(lecture, {
        timeSlotId: ts.id,
      });
      ...
      toast({
        title: "Lecture moved",
        description:
            `${arg.event.title} → ${newDay} ` +
            `${toHHMMSS(newStart).slice(0, 5)}`,
      });
      ...
      <div className="flex flex-col gap-2">
        ...
        >
          ...
          <p className="text-xs text-muted-foreground pt-2">
            Drag a lecture to a new day or time,
            or resize its edges to change duration.
