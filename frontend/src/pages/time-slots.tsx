...
function MethodSection({ method, label, color, bg, slots }: {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    // Check for time overlap with existing slots on the same days
    ...
    if (conflict) {
      ...
      toast({
        title: "Time overlap",
        description: `This window generates a slot that overlaps with the existing ${conflict.startTime?.slice(0, 5)}–${conflict.endTime?.slice(0, 5)} slot on: ${sharedDays}. Remove the conflicting slot first or choose different days/times.`,
        variant: "destructive",
      });
      return;
    }
    try {
      ...
      toast({ title: `${label} slots generated successfully` });
    } catch (err: any) {
        ...
        <CardContent className="space-y-4">
          ...
          <form onSubmit={submit} className="space-y-3">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              ...
              <div>
                <Label className="text-xs">Duration (per slot)</Label>
            ...
            <Button type="submit" size="sm" disabled={create.isPending}>
              {create.isPending ? "Generating Slots…" : "Generate Slots"}
            </Button>
          </form>
          ...
          {slots.length === 0 && (
              <p className="text-xs text-muted-foreground">No time slots generated yet.</p>
          )}
        </CardContent>
      </Card>
  );
      ...
      <div className="space-y-5">
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  This removes all {list.data?.length ?? 0} generated time slots permanently.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
        ...
        <div className="rounded-xl border border-[#1A4D35]/20 bg-[#1A4D35]/5 p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-xs font-semibold text-foreground">Auto-Generation</p>
                <p className="text-xs text-muted-foreground">
                  The system immediately divides the window into individual slots of the chosen duration. You can delete any specific slot you don't want.
                </p>
              </div>
            </div>
          </div>
        ...
        {METHOD_META.map(({ method, label, color, bg }) => (
            <MethodSection
                ...
                color={color}
                bg={bg}
                slots={list.data?.filter(t => t.teachingMethod === method) ?? []}
            />
        ))}
      </div>
  );
