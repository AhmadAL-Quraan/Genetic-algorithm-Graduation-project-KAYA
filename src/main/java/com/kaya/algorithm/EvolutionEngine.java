    ...
    public ArrayList<TimeTable> evolveGenerations(ArrayList<TimeTable> population,
        ...
        for (int gen = 1; gen <= config.maxGenerations; gen++) {
            ...
            while (nextGen.size() < config.populationSize) {
               ...
               /*
               استبدله بجملة ال if الي تحت//
                // 4. SMART MUTATION: Fitness-Proportionate Probability
                // Dynamically adjust the mutation probability based on the individual's quality.
                double individualMutationChance = currentMutationRate;

                // If the offspring's fitness is worse than the current generation's best,
                // we double its mutation chance to forcefully introduce structural changes.
                // We cap this at 0.80 (80%) to ensure it remains higher than the stagnation cap (0.50).
                if (child.getFitness() < newGenerationBestFitness) {
                    individualMutationChance = Math.min(0.80, currentMutationRate * 2.0);
                }

                // Execute mutation based on the dynamically calculated probability for this specific child
