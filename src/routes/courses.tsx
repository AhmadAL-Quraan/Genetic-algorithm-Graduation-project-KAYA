...
function CoursesPage() {
  ...
  const deleteAll = Courses.useDeleteAll();
  const [form, setForm] = useState<CourseInput>(empty);

  const set = (patch: Partial<CourseInput>) =>
    setForm((f) => ({ ...f, ...patch }));
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Pick a teaching method — the algorithm assigns a time slot automatically.
          </p>
        </div>
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  This will permanently delete all {list.data?.length ?? 0} courses.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
          ...
          <form onSubmit={submit} className="space-y-5">
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div>
                ...
                <Select
                  value={form.roomGroups}
                  onValueChange={(v: RoomType) => set({ roomGroups: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div>
                ...
                <Select
                  value={form.timeGroups}
                  onValueChange={(v: TeachingMethod) => set({ timeGroups: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {TEACHING_METHODS.map((t) => (
                      <SelectItem key={t} value={t}>
                        {t}
                      </SelectItem>
                    ))}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">
            All Courses ({list.data?.length ?? 0})
          </CardTitle>
        </CardHeader>
